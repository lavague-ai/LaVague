...
class ContextManagerExcluder(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node):
        ...
        node.body = [stmt for stmt in node.body if not (
            isinstance(stmt, ast.With) and
            isinstance(stmt.items[0].context_expr, ast.Call) and
            isinstance(stmt.items[0].context_expr.func, ast.Name) and
            stmt.items[0].context_expr.func.id == self.context_manager_name
        )]
        return node
...
class PythonExporter(TrajectoryExporter):
    ...
    def translate(cls, method: Callable, output: NavigationOutput | ExtractionOutput, string_only: bool = True) -> Optional[str]:
        ...
        source = PythonExporter.replace_code_with_value(source, output, attribute_pattern, "action_output.", string_only)
    ...
    def translate_boilerplate(cls, method: Callable, trajectory: Trajectory, string_only: bool = True) -> str:
        """
        ...
        source = PythonExporter.replace_code_with_value(source, trajectory, attribute_pattern, "trajectory.", string_only)
        return source

    @classmethod
    def replace_code_with_value(cls, source: str, object: Any, attribute_pattern: str, attribute_prefix: str, string_only: bool = True) -> str:
        """
...
class PythonSeleniumExporter(PythonExporter):
    ...
    def set_value(self, action_output: NavigationOutput) -> Optional[str]:
        with exclude_from_export():
            driver = self.get_driver()
        driver.find_element(By.XPATH, action_output.xpath).send_keys(action_output.value)

    def set_value_and_enter(self, action_output: NavigationOutput) -> Optional[str]:
        with exclude_from_export():
            driver = self.get_driver()
        driver.find_element(By.XPATH, action_output.xpath).send_keys(action_output.value + Keys.ENTER)
        ...
        for action in trajectory.actions:
            if action.action_type == ActionType.NAVIGATION:
                ...
                for output in action.action_output:
                    ...
                    translated_action_lines.append(f"\ntime.sleep({self.time_between_actions})")
