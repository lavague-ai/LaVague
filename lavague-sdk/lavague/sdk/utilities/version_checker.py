...
def check_latest_version():
    package_version = get_installed_version("lavague-sdk")
    ...
    if compare_versions(package_version, latest_version) < 0:
        warnings.warn(
            YELLOW
            + f"You are using lavague-sdk version {package_version}, however version {latest_version} is "
            "available. You should consider upgrading via the "
            "'pip install --upgrade lavague-sdk' command." + RESET,
            UserWarning,
        )
