...
def launch(ctx):
    ...
    from ..context import ActionContext, Instructions
    from ..command_center import GradioDemo

    config = ActionContext.from_path(ctx.obj["config"])
...
def build(ctx, output_file: Optional[str], test: bool = False):
    ...
    config: ActionContext = ActionContext.from_path(ctx.obj["config"])
