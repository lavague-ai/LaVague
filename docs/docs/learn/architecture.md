...
- `Instruction:` An instruction is the next step generated by the World Model, based on the current state of the webpage and the objective, needed to move towards achieving the objective. For example: `"Locate the username input field and enter the text 'user123'."`
- `World Model`: The World Model analyzes a user objective and the current state of a webpage to generate the instructions needed for achieving the objective.
- `Action Engine`: The Action Engine receives an instruction and generates the automation code required to perform this action.
- `Driver`: The chosen webdriver provides the World Model with **perception** through screenshots and HTML source code & is leveraged for the execution of the action code generated by the Action Engine.
